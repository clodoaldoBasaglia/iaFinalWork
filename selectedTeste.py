...
def extrairCaracteristicas(img):
    ...
    edge = cv2.Canny(blur, low, low * 3, 3)
    return edge.flatten()
...
def resize(caminho, arq):
    ...
    pasta = arq[0:3]
    if not os.path.exists('testeResize' + pasta):
        os.mkdir('testeResize' + pasta)
    caminhoSaida = 'testeResize' + arq
...
def main():
    separator = os.sep
    treino = open('NCharacter_SD19_BMP' + separator + 'NIST_Test_Upper.txt', 'r')
    arqTreinos = treino.readlines()
    cont = 0
    treinoFinal = open('testeFinal.csv', 'w', newline='\n')
